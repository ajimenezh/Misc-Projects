...
#  endif
	...
	template<typename Numeric>
	bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::map<std::string, std::string>& keywords = std::map<std::string, std::string>())
	{
	...
	template <typename Numeric>
	void scatter_3d(const std::vector<::std::vector<Numeric>> &x,
		const std::vector<::std::vector<Numeric>> &y,
		const std::vector<::std::vector<Numeric>> &z,
		const std::map<std::string, std::string> &keywords =
		std::map<std::string, std::string>())
	{
		...
		PyObject *scatter_3d = PyObject_GetAttrString(axis, "scatter");
		if (!scatter_3d) throw std::runtime_error("No scatter_3d");
		Py_INCREF(scatter_3d);
		PyObject *res = PyObject_Call(scatter_3d, args, kwargs);
		if (!res) {
			PyErr_Print();
			throw std::runtime_error("failed scatter 3d");
		}
		Py_DECREF(scatter_3d);

		PyObject* list = PyList_New(2);
		PyList_SetItem(list, 0, PyFloat_FromDouble(-1.0));
		PyList_SetItem(list, 1, PyFloat_FromDouble(1.0));

		PyObject* lim_args = PyTuple_New(1);
		PyTuple_SetItem(args, 0, list);

		PyObject *zlim = PyObject_GetAttrString(axis, "set_zlim3d");
		if (!zlim) throw std::runtime_error("No zlim");
		Py_INCREF(zlim);
		PyObject *kwargs2 = PyDict_New();
		PyDict_SetItemString(kwargs2, "bottom", PyFloat_FromDouble(-10.0));
		PyDict_SetItemString(kwargs2, "top", PyFloat_FromDouble(10.0));

		PyObject *res2 = PyObject_Call(zlim, detail::_interpreter::get().s_python_empty_tuple, kwargs2);
		if (!res2) {
			PyErr_Print();
			throw std::runtime_error("failed zlim");
		}
		Py_DECREF(zlim);
